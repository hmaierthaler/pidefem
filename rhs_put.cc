...
double I_rhs_put(double x, void *param){
	...
	if (bu < 0){
		ret = exp(-r*tau)*K*C*chi(-bu,1.0+Y,G) \
		      -exp(x)*C*chi(-bu,1.0+Y,G+1.0);
	} else {
		ret = (-exp(-r*tau)*K+exp(x) )*C*chi(bu,1+Y,M) + exp(x)*C*chi(bu,Y,M);
	}
...
// assemble load vector for European put option
void assemble_rhs_eur_put(	DEVector &rhs, DEVector grid, double offset,
		 	double tau, double sigma, double gamma, double lambda,
		 	double r, double K, double C, double G, double M, double Y){
	...
	for (int i=1; i<=n; i++){
		...
		if (i>1 && i<n && bu > x2){
			rhs(i) += -(gamma+0.5*sigma*sigma-r)*((exp(x1)*x0-exp(x1)*x1+exp(x1)
				-exp(x0))/(x0-x1)-(exp(x1)*x1-exp(x1)*x2+exp(x2)-exp(x1))/(x1-x2))
				+lambda*(-(x1-x2)*(x0-2)*exp(x0)+(x1-2)*(x0-x2)*exp(x1)
				-exp(x2)*(x2-2)*(x0-x1))/((x0-x1)*(x1-x2)); // OU Part
			
		}
		if (x0 < bu && bu < x1){
			rhs(i) += -(gamma+0.5*sigma*sigma-r)*(((-bu+x0+1)*exp(bu)-exp(x0))/(x0-x1))\
				+lambda*(((bu-1)*x0-bu*bu+2*bu-2)*exp(bu)-exp(x0)*(x0-2))/(x0-x1); // OU Part
		}

		if (x1 < bu && bu < x2){
			rhs(i) += -(gamma+0.5*sigma*sigma-r)*(((x0-x1)*(bu-x2-1)*exp(bu)
				+(x2-x1)*exp(x0)+(x0-x2)*exp(x1))/((x0-x1)*(x1-x2)))
				+lambda*(((-bu+1)*x2+bu*bu-2*bu+2)*(x0-x1)*exp(bu)
				-(x1-x2)*(x0-2)*exp(x0)+(x1-2)*(x0-x2)*exp(x1))/((x0-x1)*(x1-x2));
		}
	...
	for (int i=1; i<=n; i++){
		...
		if (i>1 && i<n && bu > x2){
			rhs(i) += -(gamma+0.5*sigma*sigma-r)*((exp(x1)*x0-exp(x1)*x1+exp(x1)
				-exp(x0))/(x0-x1)-(exp(x1)*x1-exp(x1)*x2+exp(x2)-exp(x1))/(x1-x2))
				+lambda*((-(x1-x2)*(x0-2)*exp(x0)+(x1-2)*(x0-x2)*exp(x1)
				-exp(x2)*(-2+x2)*(x0-x1))/((x0-x1)*(x1-x2))) -r*K*(-0.5*x0+0.5*x2);
			
		}
		if (x0 < bu && bu < x1){
			rhs(i) += -(gamma+0.5*sigma*sigma-r)*(((-bu+x0+1)*exp(bu)-exp(x0))/(x0-x1)) 
				+lambda*((((bu-1)*x0-bu*bu+2*bu-2)*exp(bu)-exp(x0)*(x0-2))/(x0-x1)) 
				+r*K*0.5*(bu-x0)*(bu-x0)/(x0-x1);
		}

		if (x1 < bu && bu < x2){
			rhs(i) += -(gamma+0.5*sigma*sigma-r)*(((x0-x1)*(bu-x2-1)*exp(bu)
				+(x2-x1)*exp(x0)+(x0-x2)*exp(x1))/((x0-x1)*(x1-x2)))
				+lambda*(((x0-x1)*((-bu+1)*x2+bu*bu-2*bu+2)*exp(bu)
				-(x1-x2)*(x0-2)*exp(x0)+(x1-2)*(x0-x2)*exp(x1))/((x0-x1)*(x1-x2))) 
				-r*K*0.5*((-2*bu+x0+x1)*x2+bu*bu-x0*x1)/(x1-x2);
		}
...
void assemble_rhs_swing_am_put(	DEVector &rhs, DEVector grid, double offset,
	...
	if ( tau >= (p-1.0)*delta){
			...
			assemble_rhs_am_put(rhs, grid, 0.0, sigma, gamma, lambda, r, K, C(tau), G, M, Y);

			assemble_rhs_swing_am_put(rhs2, grid, r*delta, tau-delta, 
				delta, p-1, sigma, gamma, lambda, r, K, C, G, M, Y);
			rhs = rhs + exp(-r*delta) *rhs2;
	}
	// p-1 exercises possible
	else if (tau >=0 && tau < (p-1.0)*delta) {
		assemble_rhs_swing_am_put(rhs, grid, 0.0, tau, 
			delta, p-1, sigma, gamma, lambda, r, K, C,G,M,Y);
	}
...
void assemble_rhs_swing_eur_put(DEVector &rhs, DEVector grid, double offset,
	...
	if ( tau >= (p-1.0)*delta){
			// 6th parameter = gamma
			assemble_rhs_eur_put(rhs1, grid, 0.0, tau, 
				sigma, gamma, lambda, r, K, C(tau), G, M, Y);

			assemble_rhs_swing_eur_put(rhs2, grid, r*delta, tau,delta, p-1, 
				sigma, gamma, lambda, r, K, C, G, M, Y);
			rhs = rhs1 + exp(-r*delta) *rhs2;
	}
	// p-1 exercises possible
	else if (tau >=0 && tau < (p-1.0)*delta) {
		assemble_rhs_swing_eur_put(rhs, grid, 0.0, tau, delta, p-1, 
			sigma, gamma, lambda, r, K, C,G,M,Y);
	}
